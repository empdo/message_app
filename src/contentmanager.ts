...
class ContentManager extends EventEmitter {
  ...
  public setLocalUserPicture = async () => {
    //TODO: do it other way becuase this is bad
  ...
  private request = async (
    ...
    const response = await fetch(
      "https://messageapi.essung.dev" + location,
      requestOptions
    );
  ...
  public addMessage = async (message: Message) => {
    const existingConvos = this.conversations.map(
    ...
    this.addFirst(
      this.conversations.find(
        (conversation) => conversation.id === message.sender
      )
    );
  ...
  public sendMessage = async (reciver: number, content: string) => {
    ...
    this.addFirst(
      this.conversations.find((conversation) => conversation.id === reciver)
    );

    await this.request(
      "POST",
      JSON.stringify({ reciver: reciver, content }),
      "/send",
      this.token
    );
  };
