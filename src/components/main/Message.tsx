...
import { useCurrentConversation } from "./main";
      ...
      <div className={"message " + classes}>
        {shouldShowTime &&
          (picture ? (
            <img
              alt=" "
              src={`https://messageapi.essung.dev/static/${picture}`}
            />
          ) : (
            <span />
          ))}
        <div className="message-container">
          {shouldShowTime && (
            <div className="message-title">
              <h3>
                {message.sender === currentConversation?.id
                  ? currentConversation?.name
                  : contentManager.user?.name}
              </h3>
    ...
    return (
      <>
        {props.messages.map((message, index) => {
          ...
          let shouldShowTime =
            !datesAreOnSameMinute(date, previousDate) ||
            !sameSender ||
            !datesAreOnSameDay(date, previousDate);
          ...
          return (
            <React.Fragment key={message.id}>
              ...
              <MessageTemplate
                ...
                classes={
                  message.sender === contentManager.user?.id
                    ? "sender"
                    : "receiver"
                }
                message={message}
